...
export default function BookingItem({ booking }: BookingItemProps) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl transition-all group overflow-hidden relative">
            ...
            <div className="mt-4 text-right">
              ...
              <div className="flex gap-1 justify-end flex-wrap max-w-[150px]">
                {booking.seats.map((seat: string) => (
                  <span
                    key={seat}
                    className="bg-primary text-white text-[10px] font-black px-2 py-1 rounded"
                  >
                    {seat}
                  </span>
                ))}
      ...
      <div className="bg-gray-50/50 p-4 border-t border-gray-100 flex justify-end">
        <button
          ...
          className="flex items-center gap-2 text-sm font-black text-primary hover:text-primary-dark transition-colors bg-white px-4 py-2 rounded-lg border border-primary/20 shadow-sm disabled:opacity-50"
        >
          {isDownloading ? (
